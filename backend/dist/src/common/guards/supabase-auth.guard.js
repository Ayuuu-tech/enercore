...
let SupabaseAuthGuard = class SupabaseAuthGuard {
    ...
    async canActivate(context) {
        ...
        if (!supabase) {
            throw new common_1.UnauthorizedException('Supabase client not initialized');
        }
        const { data: { user }, error } = await supabase.auth.getUser(token);
        if (error || !user) {
            throw new common_1.UnauthorizedException(error?.message || 'Invalid Supabase session');
        }
        request.user = {
            id: user.id,
            email: user.email,
